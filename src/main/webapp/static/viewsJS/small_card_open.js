...
async function renderSingle() {
    try {
        ...
        return res3;
        let html_left = "";
        html_left = `<pop-up></pop-up> <small-cardopen img_src="${data.introduction_media}" title="${data.course_title}" description="${data.decription}" author="${data.f_name}+${data.l_name}" price="${data.price}" date="${data.date}" content_id="${data.content_id}" ></small-cardopen><comment-list dataString="${encodeURIComponent(res3)}"> </comment-list>`;
